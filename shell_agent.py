...
class Agent:
    ...
    # conditional_edges
    def ask_human(self, state: AgentState) -> Command[Literal["action", "human_approval", END]]:
        """
        returns:
            "action"，如果需要调用工具，需要用户确认
            "human"，如果征求用户意见,直接跳过确认
            "next"，如果没有工具调用，直接进行回复
        """
    ...
    def human_approval(self, state: AgentState) -> Command[Literal["action", "human_approval", "ask_human"]]:
        message = state['messages'][-1]
        is_approved = interrupt({"question": "是否继续执行？(y/n)"})
        if is_approved:
